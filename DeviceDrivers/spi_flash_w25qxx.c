...
static uint32_t w25qxx_read(uint32_t offset, uint8_t * buffer, uint32_t size)
{
	...
	rt_spi_send_then_recv(spi_flash_device.rt_spi_device, send_buffer, 4,
			buffer, size);
...
uint32_t w25qxx_page_write(uint32_t page_addr, const uint8_t* buffer)
{
	...
	for (index = 0; index < (PAGE_SIZE / 256); index++)
	{
		...
		rt_spi_send_then_send(spi_flash_device.rt_spi_device, send_buffer, 4,
				buffer, 256);
...
static rt_err_t w25qxx_flash_control(rt_device_t dev, rt_uint8_t cmd,
		void *args)
{
...
static rt_size_t w25qxx_flash_read(rt_device_t dev, rt_off_t pos, void* buffer,
		rt_size_t size)
{
	flash_lock((struct spi_flash_device *) dev);

	w25qxx_read(pos * spi_flash_device.geometry.bytes_per_sector, buffer,
...
static rt_size_t w25qxx_flash_write(rt_device_t dev, rt_off_t pos,
		const void* buffer, rt_size_t size)
{
	...
	{       
		w25qxx_page_write(
				(pos + i) * spi_flash_device.geometry.bytes_per_sector, ptr);
		ptr += PAGE_SIZE;
		i++;
	}
...
rt_err_t w25qxx_init(const char * flash_device_name,
		const char * spi_device_name)
{
	struct rt_spi_device * rt_spi_device;

	/* initialize mutex */
	if (rt_mutex_init(&spi_flash_device.lock, spi_device_name, RT_IPC_FLAG_FIFO)
			!= RT_EOK)
	{
...
int spi_flahs_init(void)
{
	w25qxx_init("flash", "w25q64");

	return 0;
}
INIT_COMPONENT_EXPORT(spi_flahs_init);
